...
echo "   tmux new-window -n \"${TASK_NAME}\" \"cd worktree/${TASK_NAME} && PROJECT_ROOT=\$PROJECT_ROOT claude '../../.parallel-dev/tasks/${TASK_NAME}.md を読んで実装してください。完了したら \\\$PROJECT_ROOT/.parallel-dev-signals/${TASK_NAME}.done を作成してください（worktree 内ではなく親プロジェクトに作成）。'\""
echo ""
